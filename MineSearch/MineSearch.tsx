...
import {
    ReducerActions, START_GAME, OPEN_CELL, NORMALIZE_CELL,
    QUESTION_CELL, CLICK_MINE, FLAG_CELL, INCREMENT_TIMER,
} from './action';
...
} as const; // 고정된 값들을 사용하는 경우 as const 로 string 이나 number가 아니라 그 값만 가지도록!
...
export const TableContext = createContext<Context>({ // 리덕스를 사용하지 않고 순수 props를 이용하는 컴포넌트라면 사용을 해봄직 할 듯
    tableData: [],
    halted: true,
    dispatch: () => {},
});
...
const plantMine = (row: number, cell: number, mine: number): Codes[][] => {
    const candidate = Array().fill(null).map((arr, i) => { return i });
    const shuffle = [];
    while (candidate.length > row * cell - mine) {
        const chosen = candidate.splice(Math.floor(Math.random() * candidate.length), 1)[0];
        shuffle.push(chosen);
        //? 섞어서 꽂아넣는 방식 아주아주 신선함
    }
    const data: Codes[][] = [];
    for (let i = 0; i < row; i++) {
        const rowData: Codes[] = [];
